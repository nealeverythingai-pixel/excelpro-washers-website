  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={<TrendingUp className="h-5 w-5" />} label="Active" value={stats?.active ?? '—'} color="text-green-600 dark:text-green-400" />
        <StatCard icon={<Trash2 className="h-5 w-5" />} label="Unsubscribed" value={stats?.unsubscribed ?? '—'} color="text-red-600 dark:text-red-400" />
        <StatCard icon={<Send className="h-5 w-5" />} label="Campaigns Sent" value={campaignList.filter(c => c.status === 'sent').length} color="text-blue-600 dark:text-blue-400" />
      </div>

      {/* Source Breakdown */}
      {stats?.sources && Object.keys(stats.sources).length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Subscriber Sources</h3>
          <div className="flex flex-wrap gap-3">
            {Object.entries(stats.sources).map(([source, count]) => (
              <span key={source} className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs px-3 py-1 rounded-full">
                {source}: {count}
              </span>
            ))}
          </div>
      ...
      {tab === 'compose' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Compose Campaign</h2>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-green-500 focus:border-transparent focus:outline-none"
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                <button
                  ...
                  className="ml-2 text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 text-xs"
                >
              ...
              <textarea
                ...
                className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-4 py-3 font-mono text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent focus:outline-none"
              />
            </div>
      ...
      {tab === 'subscribers' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          {loading ? (
            <p className="p-6 text-gray-500 dark:text-gray-400 text-center">Loading...</p>
          ) : subscriberList.length === 0 ? (
            <p className="p-6 text-gray-500 dark:text-gray-400 text-center">No subscribers yet. They&apos;ll appear here once people sign up.</p>
          ) : (
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {subscriberList.map((sub) => (
                    <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{sub.name || '—'}</td>
                      <td className="px-4 py-3">
                        <span className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs px-2 py-0.5 rounded-full">{sub.source}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                          sub.status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' :
                          sub.status === 'unsubscribed' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
      ...
      {tab === 'campaigns' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Campaign History</h2>
          </div>
          {campaignList.length === 0 ? (
            <p className="p-6 text-gray-500 dark:text-gray-400 text-center">No campaigns sent yet.</p>
          ) : (
              ...
              {campaignList.map((campaign) => (
                <div key={campaign.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-gray-900 dark:text-white font-medium">{campaign.subject}</h3>
